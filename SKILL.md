...
`config/report-config.json`에 있다. **이 문서나 references, validate.py에
값을 다시 적지 않는다.** 값이 바뀌면 그 파일만 고친다.

문서 본문에 나오는 구체적 값(예: 개업일 2026-08-26)은 설명을 위한 예시이며,
...
OFF 그룹 키워드 10개 중 노출이 잡힌 것(예: 8/26의 `노원필라테스`)만 보이고 나머지는
...
- 04번 예산 비중 합계 = 100.0%
- 01번 차트 `min-width` = 날짜 수 × 설정값(기본 80px, 최소 650px)
...
위 목록 중 "매번 함께 바꿔야 할 텍스트" 1·3·8·9번은 이제 자동 검사 대상이다.
남은 수동 항목은 2(og:description) · 4(06번 x축 라벨) · 5(09번 심야 콜아웃) ·
6(11번 날짜 문장) · 7(01번 인사이트 박스)뿐이다.

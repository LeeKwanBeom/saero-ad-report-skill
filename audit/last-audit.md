...
| D-2 .ctr-high 규칙 | 해결됨(저장소·배포 저장소) / **라이브 미반영** | css-and-layout.md 44행 `표 무관, 클릭률 전용` · validate.py 13행 `5. 01번 표도 같은 규칙으로 전수 대조` · index.html 1287행 `01·07번 표에서 클릭률 4% 이상인 행만`. 라이브는 아직 `07번 표에서` |
...
| 배포본 1287행 문구 | 저장소 반영 확인 / 라이브 미확인→**불일치** | 위 라이브 Pages 확인 |
...
## 다음 점검에서 대조할 것
- **라이브 Pages 1287행 문구**가 `01·07번 표에서`로 바뀌었는지(이번 회차 `07번 표에서` 그대로). 계속 다르면 Pages 빌드 확인.

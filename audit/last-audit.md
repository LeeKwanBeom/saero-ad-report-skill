...
| I-4 validate 범위 | **조치완료(2026-09-07 저녁, 검사 11개)** | validate.py 인자에 상세지역 CSV 없음 |
...
다섯 개 전부 반영. 실 CSV(2026.08.26–09.06)와 배포본으로 검사 11개 전수 실행,
...
- **I-4 완료** — validate.py 검사 5개 → **11개**. 상세지역 CSV를 5번째 인자로 추가.
...
### 검사 생존 확인 [실측] — 실 CSV 기준 11개 전부 PASS 후 개별 파괴
...
부수 확인: 실 CSV로 돌린 11개 검사가 전부 PASS했다는 것은 **현재 배포본 자체가
...
- v3.3 (2026-09-07) — I-4 반영. validate.py 검사 5개→11개, 인자에 상세지역 CSV 추가.
...
- validate.py 11개 검사 중 실 갱신에서 오탐이 나는 게 있는지. 특히 masthead 형식(`YYYY.MM.DD — MM.DD (N일)`)과 예산 비중 반올림.

...
| 등록일 | 위치 | 목록(원본) | 비고 |
|---|---|---|---|
| (09-26 제안·미등록) | 파워링크 3개 그룹 "확장 검색" 칸 제안 | 노원역에새건물 · 노원역추석영업 · 6세필라테스 (+ 판단 요청: 노원역두 · 노원역월 · 노원역스튜디오) | 9/25 첫 등장 12개 중. 전부 "확장"·클릭 0. 사용자 등록 확인되면 등록일을 채우고 다음 날부터 대조 |
...
| 필라테스안 계열(노원필라테스안 9/25 2 · 필라테스안노원역 9/25 1 신규 — 9/23~9/25 사흘 6회, 누적 8·클릭 0) | **종결 건 재등장 — 채팅 확인 요청(표 미등재)** | 1회짜리 재등장 범위를 넘음(정원 09-23과 같은 처리). 웹 검색(필라테스안 노원) 소재지 특정 불가. 노원 업체면 채택(config 추가), 아니면 제외 확정 | 09-26 |

...
**다음 회차 첫 확인**: 재등록 뒤 화면 건수가 유지되는지(다시 0이면 저장 문제 → 네이버 문의), 재등록일 이후 목록 검색어 노출 0인지, 어근 부분일치 작동 여부. "계정 조치 기록 (2026-09-10)" 절의 38개 등록은 **실제로는 남아 있지 않았던 것**으로 판명 — 원문은 보존하고 여기 정정으로 남김.

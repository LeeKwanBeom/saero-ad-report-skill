...
- **9/17 계정 조치 요약(사용자 직접)**: ① 파트너 매체 설정 5개 그룹 해제 확인(스크린샷) ② 08:20 제외 검색어 어근 7개+개별 67개를 4개 그룹에 등록(9/10 38개 유지 확인) ③ 노원키즈필라테스 광고그룹 OFF ④ 파워링크 예산 현행 유지 결정. 미수령은 ② 지역 설정 현재값뿐. 산전·산후 수업 운영 여부는 아기랑 계열 제외 판단용으로 다음에 물을 것.

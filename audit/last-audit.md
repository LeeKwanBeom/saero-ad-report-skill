# 점검 기준선
점검일: 2026-09-07 (저녁 회차, Fable) — 진단만. 수정 없음. 사용자가 고를 항목 대기
결함 3건(D-7·D-8·D-9, 전부 미조치) / 개선안 5건(이월 1 + 신규 4) / 인용불가로 제외 0건
...
| 노원M필라테스 (변형 M필라테스노원·노원엠필라테스) | 채택(표) | 노원구 소재 확인. 09-07 CSV 4회·클릭 0 (같은 CSV) | 09-06 |
...
## 점검표 개정안 (채택 여부 미확인 — checklist.md는 이번 회차 변경 없음)
...
- (이번 회차 변경 없음 — 개정안 채택 전) v4.1 유지.
- v4.1 (2026-09-07) — `[검증 회차]` 절 추가. (이하 직전 회차 기록과 동일, 아래 "이전 기록" 참고)

## 다음 점검에서 대조할 것
- D-7·D-8·D-9 중 사용자가 고른 항목의 수정 여부. D-9를 코드로 고쳤으면 06번 min-width 파괴 실험(675행 960→900)이 FAIL로 바뀌는지.
- D-7을 (a)로 고쳤으면 SKILL.md 156·205–209·236행에 값이 없어야 하고, (b)로 고쳤으면 (1-1) 표에 SKILL.md 행이 있어야 한다.
- 직전 기준선 118행·112행 "전부 조치완료" 문구 정정 여부(D-9 ③).

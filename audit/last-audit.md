...
- **D-8 조치완료** — validate.py 검사 이름 4곳을 `f"… {CTR_HIGH:g}% 이상에만 …"`으로.
...
- **파괴 실험 전부 재실행(검사 조건 변경 원칙)**: 14개 검사 각 1곳씩 → 14/14 FAIL, 0건 가드 7종 FAIL(masthead·KPI·04 td·각주·01 min-width·07 name-cell·날짜형 라벨 배열 형식 변조). 완화된 검사 없음. 스크립트 `mutation-test.py` 산출물로 첨부.
